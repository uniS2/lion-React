...
// 실패한 경우: 오류 메세지를 화면에 출력

import { useEffect, useState } from "react";

// 컴포넌트 상태 (최소한의 갯수로 관리가 요구)
// 1. data : 데이터 가져오기
// 2. status : 진행 상황 'panding' | 'loading' | 'success' | 'error'
// 2-1. isLoading : 대기 시간 상태 (fetched)
// 2-2. isSuccess 대기 가져오기 성공한 상태
// 2-3. isError : 데이터 가져오기 실패한 상태

function LearnStateAndEffects() {
  const [data, setData] = useState(null);
  const [error, setError] = useState(null);
  const [status, setStatus] = useState('pending');  // 설계는 본인..몫...
