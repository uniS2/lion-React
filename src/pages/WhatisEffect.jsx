...
// 렌더 트리거(요청)을 수행하는 상태 업데이트 함수에 전달한 nextState는 다음 렌더링 시점에 반영된다.
// 그렇다면 현재 시점에서 변경될 상태에 접근할 수는 없는가?
// 아니다. 사이드 이팩트로 처리가 가능하다.
// 1. 이벤트 핸들러
// 2. useEffect 훅
...
function LearnStateAndEffects() {
  ...
  const handleIncrement  = () => {
    ...
    console.log('count in event handler', count); // 10???
  }
...
/* -------------------------------------------------------------------------- */
  ...
  // 컴포넌트 조건부 렌더링에 따라 (마운트(DOM에 추가)|언마운트(DOM에서 제거)) 
    useEffect(() => {
      ...
      // 주기 마다 실행되는 것을 멈출 수 있는 고유 키가 변환
    ...
    return function cleanup() {
      // 저지른 일(주기 마다 함수 실행)을 수습(실행되지 않도록 정리) 1->2->4->8->16...
      clearInterval(cleanupKey);
    }

    }, [] /* 의존성 배열내면 상태과 연관된 어떤 값이든 들어갈 수 있나요? 네! */);
